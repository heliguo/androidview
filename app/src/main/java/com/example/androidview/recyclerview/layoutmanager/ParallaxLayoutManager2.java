 ...
 */
public class ParallaxLayoutManager2 extends RecyclerView.LayoutManager {

    private static final String TAG = "ParallaxLayoutManager";
    ...
    public ParallaxLayoutManager2(Context context, int gap) {
        normalViewGap = dp2px(context, gap);
    }

    public ParallaxLayoutManager2(Context context) {
        this(context, 0);
    }
    ...
    private int fillHorizontalLeft(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        if (onceCompleteScrollLength == -1) {
            ...
            offsetHorizontal = childWidth / 3f;
        }
        ...
        for (int i = mFirstVisiPos; i <= mLastVisiPos; i++) {
            ...
            if (dx == 0) {
                if (i == focusPosition) {
                    ...
                    item.setTranslationX(offsetHorizontal);
                } else {
                    item.setTranslationX(-offsetHorizontal);
                }
            } else if (dx > 0) {
                if (i == focusPosition) {
                    item.setTranslationX(offsetHorizontal * fractionScale);
                } else if (i < focusPosition) {
                    item.setTranslationX(dx + offsetHorizontal * fractionScale);
                } else {
                    item.setTranslationX(dx - offsetHorizontal * fractionScale);
                }
            } else {
                ...
                if (i == focusPosition) {
                    item.setTranslationX(-fractionScale * offsetHorizontal);
                } else if (i < focusPosition) {
                    item.setTranslationX(-dx - fractionScale * offsetHorizontal);
                } else {
                    item.setTranslationX(-dx + fractionScale * offsetHorizontal);
                }
